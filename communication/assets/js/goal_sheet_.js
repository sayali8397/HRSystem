...
function goalSheetCall(){
    ...
    */
    ...
    goalSheetObj={
        ...
        KRA1:Math.max(...arr_inter_skill),
        KRA1_weightage:arr_inter_skill_goals[arr_inter_skill.indexOf(Math.max(...arr_inter_skill))],
        KRA2:Math.max(...arr_dev),
        KRA2_weightage:arr_dev_goals[arr_dev.indexOf(Math.max(...arr_dev))],
        KRA3:Math.max(...arr_conduct),
        KRA3_weightage:arr_conduct_goals[arr_conduct.indexOf(Math.max(...arr_conduct))],
        KRA4:Math.max(...arr_comm),
        KRA4_weightage:arr_comm_goals[arr_comm.indexOf(Math.max(...arr_comm))],
        KRA5:Math.max(...arr_per),
        KRA5_weightage:arr_per_goals[arr_per.indexOf(Math.max(...arr_per))],
        KRA6:Math.max(...arr_lead),
        KRA6_weightage:arr_lead_goals[arr_lead.indexOf(Math.max(...arr_lead))],
        final_rating:"00",
        verified_by:"00",
        approved_by:"00",
        status:"pending",
